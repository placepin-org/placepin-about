...
Open, widely deployed, elegantly prefix-truncating, and excellent as a database key. Its alphabet contains vowels, so its codes can and do spell things — a real problem the moment a code becomes something a person sees rather than something a database stores. Lengths also vary, so there is no fixed rhythm to speak.
