...
| **How is the script of a code known?** | A code has no version marker today. A script-rendered code must be recognisable as such from the string alone — the same rule §10 already imposes on any future extension, and the mistake v1→v2 made once already. |
...
The point of an open protocol is that it keeps working without asking anyone. A specification whose only steward is one individual account does not clear that bar: it has a single point of failure for the domain, the npm scope, the repositories and the right to say what a code means. §10 already states the requirement — **the protocol's identity must survive the domain name** — and personal ownership is the most obvious way it currently does not.
...
For anything that is not a bug report or a specification proposal — governance, implementations in other languages, script mappings, or wanting to build on this:
